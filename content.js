...
(function () {
  ...
  function showBlockOverlay(state) {
    ...
    overlayHost.addEventListener("click", (e) => {
      ...
      if (btn.dataset.act === "back") {
        ...
        if (!panel.hidden) overlayHost.querySelector(".fb-note-input").focus();
  ...
  function scheduleLoop() {
    ...
    const gap = 90000 + Math.random() * 90000;
    loopTimer = setTimeout(() => {
      safeSendMessage(
        { type: "check-blocked", host: location.hostname },
        (st) => {
          // Null response → the extension context is gone (stale tab). Stop the
          // loop entirely so this orphaned tab quietly goes idle instead of
          // retrying forever every 90–180s.
          if (st == null) {
            clearTimeout(loopTimer);
            return;
          }
          if (st.active && !st.blocked) {
            requestLine((line) => showCompanion("cheer", line));
          }
          if (st.active) scheduleLoop();
        }
      );
    }, gap);
  }
        ...
        safeSendMessage(
          ...
          (st) => {
            if (st && st.blocked) {
              ...
              removeBlockOverlay();
              showCompanion(msg.mood || "cheer", msg.text || "");
              if (st && st.active) scheduleLoop();
            }
          }
        );
      } catch (e) {
